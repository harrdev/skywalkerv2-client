...
const PlanetDetails = (props) => {
  ...
  const planet = props.planets.filter(
    (planet) => planet.name === planetName.id
  );
  const p = planet[0];
  // Filters and maps planet films URLS to match and display
  const planetFilms = props.films
    .filter((film) => p.films.includes(film.url))
    .map((film, i) => {
      return <li key={i}>{film.title}</li>;
    });
  // Filters and maps planet's residents URLS to match
  const planetResidents = props.swapiPeople
    .filter((people) => p.residents.includes(people.url))
    .map((people, i) => {
      return <li key={i}>{people.name}</li>;
    });

  const addToFave = (info) => {
    addPlanet(info, props.user);
  ...
  return (
    <div className="container2">
      <div id="buttonDiv">
        <button className="button" onClick={() => addToFave(p)}>
          Add To Favorites
        </button>
      </div>
      <h1>Name: {p.name}</h1>
      ...
      <h3>
        Featured in Films:
        {planetFilms.length > 0 ? <ul>{planetFilms}</ul> : " None"}
      </h3>
      <h3>
        Residents:
        {planetResidents.length > 0 ? <ul>{planetResidents}</ul> : " Unknown"}
      </h3>
    </div>
  );
};
